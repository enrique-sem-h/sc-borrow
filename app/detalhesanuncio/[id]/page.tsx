...
const MOCK_ANUNCIO: AnuncioDetalhes = {
  ...
  descricao: "Ideal para perfurações em concreto, alvenaria e madeira. Alta performance, empunhadura ergonômica, acompanha chave de mandril e manual técnico.",
...
export default function DetalhesAnuncioPage() {
  ...
  const handleCalendarSelect = (newRange: DateRange | undefined) => {
    setValue("dataInicio", newRange?.from ? format(newRange.from, "yyyy-MM-dd") : "", { shouldValidate: true });
    setValue("dataFim", newRange?.to ? format(newRange.to, "yyyy-MM-dd") : "", { shouldValidate: true });
  };
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-[#1a1a1a]">
        ...
        <section className="flex-1 space-y-6 w-full pt-4">
          <div className="w-full bg-[#f8f9fa] rounded-[32px] h-[450px] overflow-hidden border border-gray-100 flex items-center justify-center p-8 shadow-sm">
            {anuncio.foto_principal ? (
              <img
                src={anuncio.foto_principal.startsWith("http") ? anuncio.foto_principal : `/${anuncio.foto_principal}`}
                alt={anuncio.titulo}
            ...
            {totalDias > 0 && (
              <div className="space-y-2 border-t border-gray-100 pt-4 text-sm text-gray-500">
                <div className="flex justify-between">
                  <span>R$ {anuncio.valorDiario.toFixed(2)} x {totalDias} dias</span>
      ...
      <style jsx global>{`
        ...
        .custom-calendar .rdp-nav_button { border-radius: 8px; }
        .custom-calendar .rdp-head_cell { color: #9ca3af; font-weight: 700; }
      `}</style>
    </main>
  );
}
